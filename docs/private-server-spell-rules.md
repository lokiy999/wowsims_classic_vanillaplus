...
Useful when hunting for a specific mechanic to check/fix:

- **Per-spell damage/coefficients/cooldowns/mana cost** (the actual combat
  math): `sim/<class>/*.go`, one file per spell/ability, e.g.
  `sim/priest/mind_flay.go`, `sim/priest/holy_fire.go`. This is what all the
  "FIXED" entries below are editing.
- **Talent effects** (stat multipliers, proc auras, cost reductions granted
  by talent points): `sim/<class>/talents.go`, e.g.
  `sim/priest/talents.go` - `ApplyTalents()` and the per-talent
  `apply<TalentName>()` functions it calls.
- **Raid/world buffs** (Power Word: Fortitude, Mark of the Wild, Stamina,
  world buffs like Songflower, etc.) - the ones toggled in the Settings tab's
  "Raid Buffs"/"World Buffs" panels: `sim/core/buffs.go`.
- **Consumables** (potions, flasks, weapon imbues, and **food** - "Well Fed"
  effects like Nightfin Soup): `sim/core/consumes.go`. Food specifically is
  the `applyFoodConsumes()` function (search `Food` in that file); potions
  are `makePotionActivationInternal()`/the `make*ConsumableMCD` helpers a bit
  above it.
- **Whether a cooldown-gated consumable (potions, on-use trinkets) actually
  fires in a sim run**: it needs the "Autocast Other Cooldowns" action
  somewhere in the Rotation tab's Priority List
  (`sim/core/apl_actions_casting.go` -
  `APLActionAutocastOtherCooldowns.Execute()` calls
  `character.UpdateMajorCooldowns()`, which is what actually pops potions).
  A rotation with only `Cast X` actions and no autocast line will never use
  them, regardless of what's selected in Settings - not a bug, just a
  missing APL entry. Confirmed 2026-09-16 after the user enabled a mana
  potion in Settings and it never triggered.
- **Base race/class stats, stat dependencies** (e.g. Stamina→Health):
  `sim/core/base_stats.go` (race/class tables) and
  `sim/core/character.go` (`AddStatDependency` calls, e.g. the
  Stamina→Health `×10` conversion).
- **Item/gear stats**: pulled from `assets/database/db.json`, generated by
  `tools/database/gen_db` from the private server's own DBC dump - see
  [private-server-item-rules.md](private-server-item-rules.md) for that
  pipeline (separate from this doc, which is spells).
