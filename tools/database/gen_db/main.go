...
// Phase 6 (SM lvl-60 rework) ilvl: matches the AQ40 average (the raid that
// historically follows BWL). Mograine and Whitemane are the Cathedral wing's
// final bosses, so their drops get pinned a notch higher than the rest.
const smBaseIlvl = 78
const smBossIlvl = 80

var smBossItemIds = map[int32]bool{
	26302: true, // Aegis of the Scarlet Commander (Mograine)
	26345: true, // Scarlet Leggings (Mograine)
	26346: true, // Scarlet Chestpiece (Mograine)
	26384: true, // Helm of Zeal (Mograine)
	26428: true, // Reliquary of Light (Mograine)
	26286: true, // Scorching Judgement (Whitemane)
	26391: true, // Whitemane's Chapeau (Whitemane)
	26395: true, // Purge (Whitemane)
}
...
func main() {
	...
	// The private server renumbered ~100 set pieces (tier + PvP) to new IDs. Rename
	// them in place so the sim uses the server's IDs. From docs/parse_vplus.py.
	renumberPath := fmt.Sprintf("%s/renumber.json", inputsDir)
	if _, err := os.Stat(renumberPath); err == nil {
		raw := map[string]int32{}
		if err := json.Unmarshal([]byte(tools.ReadFile(renumberPath)), &raw); err != nil {
			panic(err)
		}
		n := 0
		for k, newID := range raw {
			oldID, _ := strconv.Atoi(k)
			it, ok := db.Items[int32(oldID)]
			if !ok {
				continue
			}
			delete(db.Items, int32(oldID))
			if _, exists := db.Items[newID]; !exists {
				it.Id = newID
				db.Items[newID] = it
			}
			n++
		}
		fmt.Printf("Renumbered %d items to private-server IDs\n", n)
	}

	db.MergeItems(database.ItemOverrides)

	// Private-server item values (stats + weapon damage) parsed from the in-game
	// tooltip dump via docs/parse_vplus.py. Authoritative: fully replaces stats.
	customItemsPath := fmt.Sprintf("%s/custom_items.json", inputsDir)
	customPhases := map[int32]int32{}
	if _, err := os.Stat(customItemsPath); err == nil {
		customDB := database.ReadDatabaseFromJson(tools.ReadFile(customItemsPath))
		customItems := make([]*proto.UIItem, 0, len(customDB.Items))
		for _, item := range customDB.Items {
			customItems = append(customItems, item)
			if item.Phase != 0 {
				customPhases[item.Id] = item.Phase
			}
		}
		db.MergeItems(customItems)
		fmt.Printf("Merged %d private-server item overrides\n", len(customItems))
	}

	// Items removed on the private server (raid content / tier sets the server does
	// not have). Generated by docs/gen_removed_items.py (retired; superseded by gen_include.py).
	removedItemsPath := fmt.Sprintf("%s/removed_items.json", inputsDir)
	if _, err := os.Stat(removedItemsPath); err == nil {
		var removed []int32
		if err := json.Unmarshal([]byte(tools.ReadFile(removedItemsPath)), &removed); err != nil {
			panic(err)
		}
		for _, id := range removed {
			delete(db.Items, id)
		}
		fmt.Printf("Removed %d items absent on the private server\n", len(removed))
	}

	// Rule 1 (docs/private-server-item-rules.md): keep only items AtlasLoot lists
	// (or dump-listed blue+ gear AtlasLoot doesn't tag AQ/Naxx). From docs/gen_include.py.
	includedPath := fmt.Sprintf("%s/included_items.json", inputsDir)
	if _, err := os.Stat(includedPath); err == nil {
		var included []int32
		if err := json.Unmarshal([]byte(tools.ReadFile(includedPath)), &included); err != nil {
			panic(err)
		}
		if len(included) > 2000 { // sanity floor: never nuke the DB on a bad input
			keep := make(map[int32]bool, len(included))
			...
			fmt.Printf("Kept %d items on the private-server allowlist\n", len(db.Items))
		} else {
			fmt.Printf("WARNING: included_items.json has only %d entries, skipping allowlist filter\n", len(included))
		}
	}

	// Location-based phase assignment (Pre-raid / ZG / MC / BWL / SM), from
	// docs/gen_phases.py. Applied per-item after GetPhaseData below.
	locationPhases := map[int32]int32{}
	phasesPath := fmt.Sprintf("%s/item_phases.json", inputsDir)
	if _, err := os.Stat(phasesPath); err == nil {
		raw := map[string]int32{}
		if err := json.Unmarshal([]byte(tools.ReadFile(phasesPath)), &raw); err != nil {
			panic(err)
		}
		for k, v := range raw {
			id, _ := strconv.Atoi(k)
			locationPhases[int32(id)] = v
		}
		fmt.Printf("Loaded %d location-based item phases\n", len(locationPhases))
	}

	// Loot source (drop location shown in the UI), matched against the private
	// server's own AtlasLoot fork. From docs/gen_sources.py. Full replacement of
	// item.Sources (proto merge only appends, so this can't be done via MergeItem).
	type sourceOverride struct {
		ZoneId    int32  `json:"zoneId"`
		NpcId     int32  `json:"npcId"`
		OtherName string `json:"otherName"`
	}
	itemSources := map[int32]sourceOverride{}
	sourcesPath := fmt.Sprintf("%s/item_sources.json", inputsDir)
	if _, err := os.Stat(sourcesPath); err == nil {
		raw := map[string]sourceOverride{}
		if err := json.Unmarshal([]byte(tools.ReadFile(sourcesPath)), &raw); err != nil {
			panic(err)
		}
		for k, v := range raw {
			id, _ := strconv.Atoi(k)
			itemSources[int32(id)] = v
		}
		db.MergeZone(&proto.UIZone{Id: 900001, Name: "World Boss", Expansion: proto.Expansion_ExpansionVanilla})
		fmt.Printf("Loaded %d AtlasLoot-matched item sources\n", len(itemSources))
	}
	...
	for _, item := range db.Items {
		...
		// Private-server phase overrides win over GetPhaseData:
		// custom_items.json (parse_vplus.py), then item_phases.json (gen_phases.py).
		if p, ok := customPhases[item.Id]; ok {
			item.Phase = p
		}
		if p, ok := locationPhases[item.Id]; ok {
			item.Phase = p
		}

		// Phase 6 (SM lvl-60 rework) items are all Epic on the server -- their old
		// retail green/blue Scarlet Monastery quality is stale.
		if item.Phase == 6 && item.Quality < proto.ItemQuality_ItemQualityEpic {
			item.Quality = proto.ItemQuality_ItemQualityEpic
		}

		// Phase 6 (SM) items carry stale pre-rework ilvl (or none at all for
		// brand-new items) -- reset to the SM tier's ilvl, matching the AQ40
		// average since SM is the server's post-BWL tier.
		if item.Phase == 6 {
			if smBossItemIds[item.Id] {
				item.Ilvl = smBossIlvl
			} else {
				item.Ilvl = smBaseIlvl
			}
		}

		if src, ok := itemSources[item.Id]; ok {
			drop := &proto.DropSource{ZoneId: src.ZoneId}
			if src.NpcId != 0 {
				drop.NpcId = src.NpcId
			}
			if src.OtherName != "" {
				drop.OtherName = src.OtherName
			}
			item.Sources = []*proto.UIItemSource{{Source: &proto.UIItemSource_Drop{Drop: drop}}}
		}
